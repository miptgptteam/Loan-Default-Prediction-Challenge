...
def build_cat_mapping(paths):
    mapping = {k: {} for k in CATEGORICAL}
    next_ids = {k: 0 for k in CATEGORICAL}
    for path in paths:
        with open(path) as f:
            ...
            for row in reader:
                for key in CATEGORICAL:
                    ...
                    if val not in mapping[key]:
                        mapping[key][val] = next_ids[key]
                        next_ids[key] += 1
    sizes = {k: next_ids[k] for k in CATEGORICAL}
    return mapping, sizes


def load_demographics(path: str, mapping, sizes):
    data = {}
    with open(path) as f:
        reader = csv.DictReader(f)
        for row in reader:
...
def train_logreg(X, y, lr=0.05, epochs=100, reg=0.001):
    ...
    for _ in range(epochs):
        for xi, yi in zip(X, y):
            ...
            for j, xij in enumerate(xi):
                weights[j + 1] += lr * (err * xij - reg * weights[j + 1])
    return weights
...
def build_features(row, demo_feats, prev_stats, demo_size):
    cid = row["customerid"]
    feats = []
    if cid in demo_feats:
        feats.extend(demo_feats[cid])
    else:
        feats.extend([0.0] * demo_size)
    loannum = int(row["loannumber"] or 0)
...
def load_data():
    mapping, sizes = build_cat_mapping(["data/traindemographics.csv", "data/testdemographics.csv"])
    demo = load_demographics("data/traindemographics.csv", mapping, sizes)
    demo_size = 1 + sum(sizes.values()) + 2
    ...
    with open("data/trainperf.csv") as f:
        reader = csv.DictReader(f)
        for row in reader:
            X.append(build_features(row, demo, prev, demo_size))
    ...
    return X, y, mapping, sizes, prev, mins, ranges, demo_size


def load_test_data(mapping, sizes, prev_stats, mins, ranges, demo_size):
    X = []
    ids = []
    demo_test = load_demographics("data/testdemographics.csv", mapping, sizes)
    ...
    with open("data/testperf.csv") as f:
        ...
        for row in reader:
            cid = row["customerid"]
            feats = build_features(row, demo_test, prev_stats, demo_size)
...
def cross_val_score(X, y, k=5, epochs=100):
...
def main():
    X, y, mapping, sizes, prev, mins, ranges, demo_size = load_data()
    score = cross_val_score(X, y)
    print(f"CV accuracy: {score:.4f}")

    weights = train_logreg(X, y)
    ids, X_test = load_test_data(mapping, sizes, prev, mins, ranges, demo_size)
